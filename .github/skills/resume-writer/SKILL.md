...
**问题 3 - 技术侧重**（多选）：
- 单一 Operations Agent 编排 / Tool Gateway 受控工具调用 / 预约状态机与技师匹配 / 用户记忆与偏好学习 / 自我沉淀与反思 / RAG 咨询与知识库 / MCP 知识层接入 / FastAPI 流式响应 / 分层架构与数据库 / 测试与工程化
- 如果用户选择 Agent Engineer，建议默认勾选：单一 Operations Agent 编排、Tool Gateway 受控工具调用、用户记忆与偏好学习、MCP 知识层接入。
...
| 岗位方向 | 优先亮点 |
|---------|---------|
| Agent Engineer | 亮点1(单一 Operations Agent 编排) -> 5(用户记忆) -> 6(反思闭环) -> 9(RAG/MCP 知识层) -> 8(流式体验) |
| LLM Application Engineer | 亮点1(单 Agent 工具编排) -> 2(预约状态与匹配) -> 4(RAG 咨询) -> 7(模型 Provider/Embedding) -> 8(流式响应) |
...
示例：
- "目标构建一个面向本地服务业的单一 Operations Agent 系统，通过 LangGraph 状态机、Tool Gateway 受控工具、预约状态管理、RAG 咨询和用户偏好记忆，降低前台重复沟通成本并提升预约推荐准确性。"
...
3. 夹带关键词：LangGraph、LangChain、Tool Gateway、Tool Calling、RAG、FAISS、Embedding、MCP、FastAPI、SQLAlchemy、Streaming、Repository Pattern
...
**过程**：
• [Bullet 1：单一 Operations Agent/架构/业务主线] - 动词开头 + 技术细节 + 效果
...
3. 主动提供 3-5 条面试追问预测，例如：
	- "OperationsAgent 如何维护多轮状态？LangGraph 节点如何避免状态混乱？"
	- "预约工具链怎么从自然语言里提取时间、时长、技师偏好？失败时怎么追问？"
...
| Agent 架构 | 强调单一 Operations Agent 编排、LangGraph 状态机、Tool Gateway 受控工具调用 | 声称已有复杂 P2P 子 Agent 通信 |
