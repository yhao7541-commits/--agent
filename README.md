...
当前可复现证据分为两类：`scripts/demo_decision_resilience.py` 提供不需要 API key 的确定性韧性演示；`harness.runners.run_decision_comparison` 提供需要显式配置凭据的可选的真实模型语义对比。2026-07-22 使用 `qwen-plus` 对冻结的 30 条 long-tail 案例完成了一次真实对比：25/25 个模型必需案例均成功返回结构化结果，fallback 为 0%。意图准确率由 rules 的 43.3% 提升到 hybrid 的 76.7%，歧义识别由 63.3% 提升到 83.3%，槽位召回由 0% 提升到 72.0%；但 hybrid 槽位精度只有 39.1%，仍有明显误提取 bad cases，因此不声明整体准确率提升，也不把下方 smoke 指标解释为 LLM 效果。

该次真实运行的 hybrid p50 延迟为 1,849 ms，p95 为 3,276 ms；25 个模型案例共使用 19,377 input tokens 和 2,457 output tokens。由于没有配置输入/输出 token 单价，成本状态为 `pricing_unavailable`，不估算金额。详细口径与 bad cases 见 `docs/evaluation.md`。
...
- 模型输出具有非确定性；当前真实模型证据仅来自 2026-07-22 的 30 条冻结案例对比，不代表线上业务结果，也不能证明所有槽位场景优于规则方案。
